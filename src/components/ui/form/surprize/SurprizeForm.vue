...
const onFocusField = (field: keyof SurprizeForm) => {
  resetFocusForm()
  const startPositionCursor = Number((form.value[field].ref as HTMLInputElement).selectionStart)
  const endPositionCursor = Number((form.value[field].ref as HTMLInputElement).selectionEnd)
...
<template>
  <Form :no-validate="true" @on-submit="onSubmit" class="form">
    ...
    <div class="actions">
      <Button type="button" background="primary" outline title="На главную" @on-click="onToMain"/>
      <Button type="submit" background="primary" title="Отправить" :is-loading="isLoading"/>
    </div>
  </Form>
</template>
